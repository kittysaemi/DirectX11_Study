...
bool CSoundClass::PlayWaveFile()
{
	...
	if(!m_bSoundPause)
	{
		...
		hResult = m_secondaryBuffer[0]->SetCurrentPosition(0);
		if(FAILED(hResult))
			return false;

		hResult = m_secondaryBuffer[1]->SetCurrentPosition(0);
		if(FAILED(hResult))
			return false;
	}


	hResult = m_secondaryBuffer[0]->SetVolume(DSBVOLUME_MAX);
	if(FAILED(hResult))
		return false;

	hResult = m_secondaryBuffer[0]->Play(0, 0, DSBPLAY_LOOPING);
	if(FAILED(hResult))
		return false;

	hResult = m_secondaryBuffer[1]->SetVolume(DSBVOLUME_MAX);
	if(FAILED(hResult))
		return false;

	hResult = m_secondaryBuffer[1]->Play(0, 0, DSBPLAY_LOOPING);
	if(FAILED(hResult))
		return false;
...
bool CSoundClass::PauseWaveFIle()
{
	if(!m_bSoundPlay)
	{
...
bool CSoundClass::StopWaveFile()
{
	...
	HRESULT hResult = m_secondaryBuffer[0]->Stop();
	if(FAILED(hResult))
		return false;

	hResult = m_secondaryBuffer[1]->Stop();
	if(FAILED(hResult))
		return false;
...
char* CSoundClass::GetPlayStatus()
{
	if(m_bSoundPlay)
		return "Play";

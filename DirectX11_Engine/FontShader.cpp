...
bool CFontShader::Initialize(ID3D11Device* _device, HWND _hWnd)
{
	// Initialize the vertex and pixel shaders.
	if(!InitializeShader(_device, _hWnd, L"../DirectX11_Engine/HLSL/font.vs", L"../DirectX11_Engine/HLSL/font.ps"))
	{
...
bool CFontShader::Render(ID3D11DeviceContext* _deviceContext, int idxCount, D3DXMATRIX mWorld, D3DXMATRIX mView, D3DXMATRIX mProjection, ID3D11ShaderResourceView* texture, D3DXVECTOR4 fcolor )
{

	if(!SetShaderParameters(_deviceContext, mWorld, mView, mProjection, texture, fcolor))
	{
		return false;
	}
...
bool CFontShader::InitializeShader(ID3D11Device* _device, HWND hWnd, WCHAR* vsFileName, WCHAR* psFileName)
{
// compile the vertex shader code.
	ID3D10Blob* _errMsg = 0;
	ID3D10Blob* _vertexShaderBuf = 0;
	HRESULT hResult = D3DX11CompileFromFile(vsFileName, NULL, NULL, "FontVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &_vertexShaderBuf, &_errMsg, NULL);
	...
	// compile the pixel shader code.
	  ...
	  hResult = D3DX11CompileFromFile(psFileName, NULL, NULL, "FontPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &_pixelShaderBuf, &_errMsg, NULL);
	  if(FAILED(hResult))
	  {
		  if(_errMsg)
	...
	// Create the vertex input layout description.
	  ...
	  */
	  ...
	  pixelBufDesc.ByteWidth = sizeof(PixelBufferType);
	  ...
	  hResult = _device->CreateBuffer(&pixelBufDesc, NULL, &m_pixelBuffer);
	  if(FAILED(hResult))
		  return false;
...
void CFontShader::OutputShaderErrorMessage(ID3D10Blob* _errMsg, HWND _hWnd, WCHAR* _fileName)
{
...
bool CFontShader::SetShaderParameters(ID3D11DeviceContext* _deviceContext, D3DXMATRIX mWorld, D3DXMATRIX mView, D3DXMATRIX mProjection, ID3D11ShaderResourceView* textrue, D3DXVECTOR4 fcolor)
{
	...
	hResult = _deviceContext->Map(m_pixelBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &_mappedSubResource);
	if(FAILED(hResult))
		return false;

	PixelBufferType* ptrPixelBuf = (PixelBufferType*)_mappedSubResource.pData;
	ptrPixelBuf->pixelColor = fcolor;

	_deviceContext->Unmap(m_pixelBuffer, 0);

	nbufNumber = 0;
	_deviceContext->PSSetConstantBuffers(nbufNumber, 1, &m_pixelBuffer);

	return true;
}

void CFontShader::RenderShader(ID3D11DeviceContext* _deviceContext, int _idxCount)
{
